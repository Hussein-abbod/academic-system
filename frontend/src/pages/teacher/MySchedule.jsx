...
const MySchedule = () => {
  ...
  // Only active courses that have schedule info
  const scheduled = courses.filter((c) => c.is_active);

  // Group courses by day for the weekly grid
  const byDay = {};
  ALL_DAYS.forEach((d) => { byDay[d] = []; });
  scheduled.forEach((course) => {
    const days = parseDays(course.schedule_days);
    days.forEach((d) => {
      if (byDay[d]) byDay[d].push(course);
    });
  });

  const hasAnySchedule = scheduled.some(
    (c) => parseDays(c.schedule_days).length > 0 || c.start_time
  );
    ...
    <div className="space-y-8">
      ...
      <div>
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
          <Calendar className="text-purple-500" size={26} />
          My Schedule
        </h1>
        <p className="text-gray-500 dark:text-gray-400 mt-1">
          Your weekly teaching schedule across all active courses
        </p>
      </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className="p-2.5 bg-purple-50 dark:bg-purple-900/20 text-purple-600 rounded-xl shrink-0">
                        <BookOpen size={20} />
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-gray-900 dark:text-white truncate">{course.name}</p>
